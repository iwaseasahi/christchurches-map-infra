...
resource "aws_ecs_task_definition" "db-migrate" {
  family                = "christchurches-map-db-migrate-task"
  container_definitions = templatefile("task-definitions/christchurches-map-db-migrate-task.json", { account_id = data.aws_caller_identity.current.account_id, secretsmanager_name = var.aws_secretsmanager_name })
  cpu                   = "1024"
  execution_role_arn    = "arn:aws:iam::${data.aws_caller_identity.current.account_id}:role/ecsTaskExecutionRole"
  memory                = "3072"
  requires_compatibilities = [
